...
(function (fourShadows, $, undefined) {
		...
		fourShadows.caster = function(getHour) {
			var time = new Date(),
				hour = time.getHours(),
			...
			// 00:00 - 03:59 | 13:00 - 15:59
			// 04:00 - 05:59 | 16:00 - 18:59
			// 07:00 - 08:59 | 19:00 - 21:59
			// 10:00 - 11:59 | 22:00 - 23:59

			// Direction of shadow is based on direction of the hour hand in an analog clock.
		...
		fourShadows.simulator = function() {
			hour = 00;
			var simulateHour = setInterval(function() {
				hour = hour + 1;
				fourShadows.caster(hour);
				$('#time span').text(hour + ":00");
				if(hour >= 23){
					clearInterval(simulateHour);
					$('#button').show();
				}
			},1000);
		}
